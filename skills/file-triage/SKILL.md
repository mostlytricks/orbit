---
name: file-triage
description: Sort a messy directory (usually 00-inbox) into the orbit area structure per FILING.md — plan first, move never delete, leave unclassifiable files behind with a question.
---

# file-triage

Files every item in a target directory into the orbit areas, following the filing
contract. The contract lives in `FILING.md` at the orbit root — **this skill contains
no filing rules of its own**; if sorting behavior should change, change FILING.md.

## Procedure

1. **Load the contract.** Read `FILING.md` from the orbit root (walk up from the
   target directory if needed). If it can't be found, stop — never sort without it.
...
3. **Classify** each file using FILING.md's decision procedure, in its order
