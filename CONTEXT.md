...
- Five skills: four filing-side — `file-scout` (intake) → `file-triage` (sort) → `area-architect` (restructure, also emits area cards) → `orbit-dashboard` (monitor); plus `process-architect` (documents how a process works). The four filing skills derive from `.gravity/filing/SPEC.md`; none carry filing rules.
