...
## Next Step
- **First Light** (still the now-slice): run `build-orbit-deck` (Actions → Run workflow) and launch Deck; run scout+triage on the real Desktop/Downloads — and now also start the daily habit for real (`python skills/daily-note/new_note.py`, then Friday `python skills/weekly-report/generate.py`); cut **v0.5.0** and push tags. Weekly: `python skills/orbit-janitor/sweep.py`. Report what breaks — next build slice is the Deck triage cockpit.
