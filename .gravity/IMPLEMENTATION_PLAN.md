...
- **Three materials, locked (2026-07-12)** — contracts in markdown (no runtime, readable in year 8) · labor as Python-stdlib CLIs + agent skills (the agent can read, run, and safely modify them under the walls; zero-install on the work machine) · viewers as generated self-contained HTML + Deck embedding them (documents first — they open in a bare browser with no runtime). Every mechanical piece is a data-in/artifact-out CLI behind a wall, so the runtime is **swappable per-tool, never big-bang**: if a measured bottleneck appears, that one script may be rewritten (e.g. in Rust) keeping the same CLI and the same gate. A whole-app migration to Electron-core or Rust-core is rejected: it would end the agent-as-labor property and re-prove seven walls for zero new capability.
