...
| next | **Daily loop** — `daily-note` skill (templated worklog auto-placed in `10-daily/YYYY/MM/`; likely mints a `notes` domain) then `weekly-report` (assembled from the week's notes + what actually changed in the tree). The habit-forming half of "work OS". | ○ |
| later | `orbit-janitor` — scheduled weekly sweep (inbox age, dupe waste, structure lint, stale waypoints) → one short report dropped in `10-daily/`; the tree nags instead of waiting to be asked | ○ |
| later | **Deck triage cockpit** — inbox rows with the agent's *proposed* home each; approve per-row, meteors fly as files leave. Plan-first rule as UI. | ○ |
